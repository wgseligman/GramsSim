...
bool GramsG4Options::ParseOptions(int argc, char** argv, const std::string& a_programName)
{
  bool success = true;
	  ...
	  if ( search == std::string::npos ) {
	    ...
	    if ( i+1 < argc )
	      ...
	      description << "File " << __FILE__ << " Line " << __LINE__ << " " << std::endl
			  << "missing XML file name after '--options' ";
	      G4Exception("GramsG4Options::ParseOptions()","invalid option",
  ...
  if ( m_options.size() > 0 ) {
	...
	if ( char(result) == '?' ) {
	  G4ExceptionDescription description;
	  description << "File " << __FILE__ << " Line " << __LINE__ << " " << std::endl
		      << "Option on command line not found in XML file ";
	  G4Exception("GramsG4Options::ParseOptions","invalid option",
	...
	if ( search == m_options.end() )
	  {
	    ...
	    description << "File " << __FILE__ << " Line " << __LINE__ << " " << std::endl
			...
			<< " but is not present in the options map";
	    G4Exception("GramsG4Options::ParseOptions","options error",
			JustWarning, description);
	    success = false;
	    break;
	  }
	...
	if ( type != e_boolean  &&  result == ':' ) {
	  ...
	  description << "File " << __FILE__ << " Line " << __LINE__ << " " << std::endl
		      ...
		      << " on the command line"
		      << G4endl << "   Using the value '" << (*search).second.value
		      << "'";
	  G4Exception("GramsG4Options::ParseOptions","argument missing",
		      JustWarning, description);
	  success = false;
	  break;
	}
	...
	switch (type)
	  ...
	  case e_double:
	    try {
	      ...
	      description << "File " << __FILE__ << " Line " << __LINE__ << " " << std::endl
			  ...
			  << G4endl << "   Using the value '" << m_options[name].value
			  << "'";
	      G4Exception("GramsG4Options::ParseOptions","invalid argument",
			  JustWarning, description);
	      success = false;
	    }
	    break;
	  ...
	  default:
	    ...
	    description << "File " << __FILE__ << " Line " << __LINE__ << " " << std::endl
			...
			<< G4endl << "   Option will be ignored. ";
	    G4Exception("GramsG4Options::ParseOptions","options map error",
			JustWarning, description);
	    success = false;
	    break;
	  } // switch
...
bool GramsG4Options::GetOption(const std::string name, double& value) const
{
...
bool GramsG4Options::GetOption(const std::string name, int& value) const
{
  auto result = m_options.find( name );
  if ( result != m_options.end() )
    {
...
bool GramsG4Options::GetOption(const std::string name, bool& value) const
{
  auto result = m_options.find( name );
  if ( result != m_options.end() )
    {
...
bool GramsG4Options::GetOption(const std::string name, std::string& value) const
{
  auto result = m_options.find( name );
  if ( result != m_options.end() )
    {
...
void GramsG4Options::PrintOptions() const
{
...
void GramsG4Options::PrintHelp() const
{
...
bool GramsG4Options::m_ParseXML(const std::string a_xmlFile, const std::string a_program)
{
  bool success = true;
  ...
  for (auto current_node = walker->nextNode(); 
	...
	if ( element == NULL ) {
	  std::cerr << "File " << __FILE__ << " Line " << __LINE__ << " " << std::endl
                    << "ParseXML: Could not convert DOMNode* into DOMElement*" << std::endl
		    << "   XML processing halted" << std::endl;
	  return false;
	}
