...
namespace gramselecsim {
    ...
    PreampProcessor::PreampProcessor(int size_waveform)
    {
        ...
        header_preamp_  = optionloader_->PreampHeader();
        ...
        //response of a preamp should be one?        
        for(int i=0;i<response_length_bin_;i++){
            ...
            if(header_preamp_.preamp_func==0){
                ...
                std::cout << "not validated preamp response function flag" << std::endl;
            }
        }

        //peak_delay_bin is the number of bin between peak time of a response function and when an e- cluster arrives
