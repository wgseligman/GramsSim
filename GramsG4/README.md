...
An example is the number of events to generate. 

- There's an option in the [options.xml](../options.xml) file that can control this:
...
- The final control comes from the G4 macro file. If there's a `/run/beamOn` command in the macro, it will take precedence over either of the two above methods:

        /run/beamOn 1000

As another example, consider the parameter that controls the size of
the steps of charged particles in the liquid argon; this defines the
maximum length of the "hits" in the LAr TPC.
...
A third example: Controlling the number execution threads when running `GramsG4` as a multi-threaded application.
...
If you want to change the primary particle type, energy, distribution,
or other aspects of events generated by the simulation, edit a G4 macro
file. For example, if you want to generate electrons instead of gammas, you can copy
an existing file and edit it:

```
    cp mac/batch.mac my_test.mac
    emacs my_test.mac
    # edit '/gps/particle gamma' to '/gps/particle e-'
